...
---@class reform.open_link.Event
---@field line integer
---@field column integer
---@field mouse? true whether event was generated by a click

---@alias reform.open_link.Handler fun(uri:string,ev:reform.open_link.Event):nil|1 1 for failure

---@class reform.open_link.Config
---@field unknown? 'noop'|'definition'|'copy-default'|'copy-current'|'print-default'|'print-current'|reform.open_link.Handler when nothing is matched
---       - copy|print operate with git link generated for current cursor position
---         - default|current determine which branch to use in the link
...
---@field handlers? {[1]:string,[2]:reform.open_link.Handler}[] regex & matched link handler pairs, use defaults.handlers[] to pick and order the default handlers
...
---@field open_link fun(buf:integer,ev:reform.open_link.Event) use setup handlers to open link at line:col
