...
local mcp = manipulator.call_path = {
    ...
    fn = fun(),
    new = fun(),
    region = fun(opts = manipulator.Region.current.Opts):manipulator.call_path.region|{ [''] = fun(...?):manipulator.call_path.region|{ [''] = manipulator.call_path.region }|manipulator... },
    ts = fun(opts = manipulator.ts.current.Opts):manipulator.call_path.ts|{ [''] = fun(...?):manipulator.call_path.ts|{ [''] = manipulator.call_path.ts }|manipulator.call_path.ts|{ [...] } },
    __call = fun(),
    __index = fun(),
}
```
